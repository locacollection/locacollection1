...
let orders=[],customers=[],profiles=[],orderItems=[],products=[],orderActions=[];let activeCustomer=null;
...
function openOrderWorkflow(id){
  ...
  document.getElementById("workflowMeta").textContent=`${c.name||"Guest customer"} · ${c.phone||c.email||"No contact"}`;
...
async function openCustomerProfile(encoded){
  ...
  title.textContent=c.name||"Guest customer";
...
function viewOrder(id){
  ...
  document.getElementById("modalBody").innerHTML=`<div class="order-detail-workflow"><div><small>Fulfilment</small>${orderStatusBadge(order.status)}</div><div><small>Payment</small>${paymentStatusBadge(order.payment_status)}</div><button class="smallbtn workflow-button" onclick="openOrderWorkflow('${order.id}')">Manage workflow</button></div><div class="grid"><div class="detail"><span>Customer</span>${esc(c.name||"Guest customer")}</div><div class="detail"><span>Phone</span>${esc(c.phone||order.phone||"Not provided")}</div><div class="detail"><span>Email</span>${esc(c.email||order.email||"Guest checkout")}</div><div class="detail"><span>City</span>${esc(c.city||order.city||"")}</div><div class="detail"><span>Payment method</span>${esc(order.payment_method||"COD")}</div><div class="detail"><span>Last workflow update</span>${new Date(order.status_updated_at||order.created_at).toLocaleString("en-PK")}</div><div class="detail" style="grid-column:1/-1"><span>Address</span>${esc(c.address||order.address||"Not provided")}</div></div>${cancelled}${customerActionTimeline(order.id)}<div class="order-line-items">${its.map(item=>`<div class="item"><span>${esc(item.product_name||item.name||"Product")} × ${Number(item.quantity||item.qty||0)}</span><b>${money(Number(item.unit_price||item.price||0)*Number(item.quantity||item.qty||0))}</b></div>`).join("")}</div><div class="total"><span>Total</span><b>${money(order.total)}</b></div>`;
