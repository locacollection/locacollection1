...
const originalOpenCheckout=openCheckout;openCheckout=async function(){if(LOCA.profile?.role==='admin'){LOCA.notice?.({eyebrow:'Catalogue Preview Mode',title:'Checkout is disabled for admins.',message:'Admins can inspect the storefront but cannot place orders.',action:'Continue browsing'});return}return originalOpenCheckout()};
